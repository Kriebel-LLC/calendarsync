...
export const GET = routeHandler(async (req, user) => {
  ...
  const connections = await db()
    .select({
      id: airtableConnections.id,
      airtableUserId: airtableConnections.airtableUserId,
      createdAt: airtableConnections.createdAt,
    })
    .from(airtableConnections)
    .where(eq(airtableConnections.orgId, orgId));
